...
def single_reward_threat(record):
    ...
    card_tpl = relpath("threats-1reward.svg")
...
def multi_reward_threat(record):
    ...
    card_tpl = relpath("threats-4reward.svg")
    penalty_data = split_to_multiple_fields(
        record["Penalty"],
        field_name="Penalty",
        chars_per=25,
...
def sheet_image_sample(client, output_path, ext="png"):
    rendered = sum(render(client).values(), [])
